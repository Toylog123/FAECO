...
给定映射网表 $G'$、目标周期 $T$、时序约束和工艺库 $\mathcal{L}$，FAECO 在违例端点的扇入锥上搜索局部候选 $p$，得到修复网表 $G^*$。主实验以 $\Delta_{\mathrm{WNS}}(p)>0$ 作为理想线网时序接受谓词，并要求修复前后网表保持功能等价；启用物理门控时，还需满足 §3.5 所述 SPEF 复测条件。可选的 TNS 感知配置仅在 WNS 持平且 TNS 改善时接受候选。四类局部变换为：R（布尔函数等价替换）、G（驱动尺寸调整）、B（恒等缓冲器插入）和 JOINT（联合枚举）。顺序实验不修改 D 触发器、时钟或复位单元。
...
\begin{enumerate}
  ...
  \item[(c)] \textbf{验证与接受}：候选经库函数和结构规则筛选后，由 OpenSTA 在理想线网模式下测量 WNS/TNS；启用物理门控时，仅理想线网增益超过 10\,ps 的候选进入简化 SPEF 复测，$0<\Delta\mathrm{WNS}\le10\,$ps 的候选不进入最终接受集合。
\end{enumerate}
...
\Repeat
  ...
  \State 若候选满足式~\eqref{eq:accept}，并在启用物理门控时通过简化 SPEF 复测，则将其加入 $\mathcal{A}$；若其 $\Delta_{\mathrm{WNS}}$ 优于当前最佳值，则相对基线网表 $G_0$ 应用该候选并更新 $G^*$。未通过接受判据的候选依据 F1--F6 失败类型更新对应权重。各轮候选均相对固定基线 $G_0$ 评估，不叠加多个局部补丁。
