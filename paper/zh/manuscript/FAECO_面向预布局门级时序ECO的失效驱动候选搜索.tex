...
\end{aligned}
\end{equation}
启用物理门控时，满足式~\eqref{eq:accept} 且理想线网增益超过 10\,ps 的候选进一步接受简化 SPEF 复测；仅当复测下 WNS 仍改善时进入接受集合 $\mathcal{A}$，否则记为 F6 并反馈至下一轮搜索。未启用物理门控时，SPEF 结果作为独立物理复测，不改变理想线网候选的接受判定。
