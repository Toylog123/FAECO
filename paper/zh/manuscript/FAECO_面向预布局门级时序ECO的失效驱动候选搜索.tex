...
\end{center}
\begin{flushleft}
  {\heiti\fontsize{8.5}{13}\selectfont 摘\quad 要：}{\songti\fontsize{8.5}{13}\selectfont 针对预布局门级时序 ECO 中缓冲器插入和门尺寸调整难以压缩深层逻辑的问题，本文提出失效驱动候选搜索引擎（Failure-Aware ECO，FAECO）。该方法结合多特征加权割与关键路径覆盖割生成候选，并根据候选失败类型更新后续搜索。候选先经库函数和结构规则筛选，再由 OpenSTA 在理想线网模式下测量时序收益；满足条件的候选进入基于标准寄生交换格式（SPEF）的简化复测。在三组测试集上，FAECO 分别实现 8/8、18/19 和 2/3 个电路的严格 WNS 改善；ITC-99 中其余 1 个电路与基线持平，全部 19 个电路均无 WNS 回退。独立逻辑门级顺序 SEC 对 29/30 个基线—修复对完全证明，b17 剩余 1 个未证明点经 Liberty 检查确认为同函数尺寸替换。ISCAS89 20 轮收敛配置下混合策略平均 WNS 改善为 1.07\,ns；另在效率优先配置对应的 OpenROAD 布局及全局布线寄生估计验证中，5/8 个电路仍保持改善。}
\par
\vspace{4pt}
...
\begin{enumerate}
  ...
  \item \textbf{功能—时序—物理分层验证}：将局部函数/结构筛选、理想线网 WNS 判定和简化 SPEF 复测串联起来，并以独立全网表 SEC 验证最终修复结果；29/30 个基线—修复对由顺序 SEC 完全证明，b17 剩余 1 个未证明点经 Liberty 检查确认为同函数尺寸替换。
...
\caption{FAECO 机制原理：局部扇入锥的特征中，边界/尺寸/验证/扇出写入加权最小割的节点代价(式(2))，关键路径覆盖不进入节点代价而通过独立覆盖割与 $\lambda_c$ 反馈形成补充候选；R 候选可用性驱动策略切换。}
...
\textbf{环境与数据集}：实验在 WSL2 Ubuntu 上完成，主流程采用 OSS-CAD Suite（Yosys 0.67+146、OpenSTA 3.1.0），顺序 SEC 采用 WSL 环境中的 Yosys 0.33。测试集包含 8 个 ISCAS89、19 个 ITC-99 和 3 个 PicoRV32 子模块，统一使用 0.5 ns 时钟约束。主要评价指标为 WNS、全局布线寄生估计后的 WNS 和候选级 STA 次数；TNS 用于可选的辅助接受判据。本文报告的实验均采用 WNS 严格改善配置；TNS-aware 仅作为可选扩展，不与本文物理门控实验联合启用。

\textbf{实验配置}：所有电路采用相同主流程和默认参数，比较效率优先和收敛两种配置；b17 的验证预算例外见 \S\ref{sec:itc99}。理想线网阶段以 WNS 严格改善为初筛条件；启用物理门控时，最终接受还需满足 §3.5 的 10\,ps 阈值和 SPEF 复测条件。完整脚本和参数随补充材料一同发布。
...
其中大规模电路 b18 和 b19 的进一步统计如下：在 0.5 ns 时钟周期下，b18 的 WNS 由 $-13.34$ ns 改善至 $-13.27$ ns（+$0.07$ ns），b19 由 $-17.44$ ns 改善至 $-16.00$ ns（+$1.44$ ns）；两者分别含 37.6 万和 75.5 万个门，经 93 次和 58 次候选级 STA 测量完成修复，测量次数保持在数十次量级。b18、b19 亦包含在上述 19 个 ITC-99 电路统计中，此处仅作门数与测量成本补充说明。
...
三组数据集的 WNS 严格改善比例分别为 8/8、18/19 和 2/3；SEC 完全证明比例分别为 8/8、18/19 和 3/3。b17 的修复网表在 SKY130 行为单元模型下经 Yosys \texttt{equiv\_simple} + \texttt{equiv\_induct} 验证，12812 个等价点已证明，剩余 1 个未证明点对应 nor4b$_1\!\to\!$nor4b$_2$ 的同函数尺寸替换（Liberty function 一致）；鉴于该点未被顺序 SEC 完全消解，本文将其单独报告。
...
\begin{tabular}{lrrrr}
\toprule
\textbf{电路} & \textbf{纯 G (固定权重)} & \textbf{纯 B (固定权重)} & \textbf{随机 (3 种子均值)} & \textbf{混合 20 轮 (有反馈)} \\
\midrule
...
\bottomrule
\multicolumn{5}{p{0.92\columnwidth}}{\footnotesize 数字为 WNS 改善量 (ns)，正值代表修复后 WNS 较基线提高；纯 G / 纯 B / 随机三列采用固定搜索权重，不启用 F1--F6 失效反馈，混合 20 轮列启用失效反馈。各方法均采用式~\eqref{eq:accept} 的候选接受准则，并使用相同的 20 轮上限与候选评估预算。随机列在与混合策略相同的 R/G/B/JOINT 候选池内随机候选评估顺序，3 种子（20260814、20260815 和 20260816）取均值。}\\
\end{tabular}
\end{table}
...
本节物理验证采用 \S\ref{sec:eff_prio} 效率优先配置得到的接受候选，与 \S\ref{sec:ablation} 的 20 轮收敛配置分开统计。在 OpenROAD 2.0 布局与全局布线（global routing）寄生估计后，8 个 ISCAS89 电路中有 5 个电路保持时序改善，1 个持平，2 个轻微恶化但不超过 0.02 ns（表~\ref{tab:pr8}）。流程使用 OpenROAD 2.0、SKY130 HD 和 0.5 ns 时钟约束；SPEF 负载扫描和迭代式门控用于量化线载影响，并将 F6 复测失败接入权重更新。

\FloatBarrier

独立 SPEF 负载扫描显示，s382 的 R 候选与 b18 的 JOINT 候选的 WNS 改善随估计线长增大而归零，表明线载增大后理想线网收益逐步衰减。

\textbf{迭代式门控}：在 40\,$\mu$m/unit 估计线载下（unit 表示一次扇出后沿拓扑/曼哈顿距离累加的相对单位长度，配合简化 RC 换算用于预布局线载估计），s382 的 50 个与 b18 的 6 个候选均未通过 SPEF 复测，并按 F6 更新权重；门控依据 SPEF 结果筛除物理无效候选并更新搜索权重。简化 SPEF 的生成规则与单位换算随补充材料一同发布。

\FloatBarrier

\begin{table}[ht]
\centering
\caption{OpenROAD 布局及全局布线寄生估计后的物理验证：8 个 ISCAS89 电路 WNS 对比（OpenROAD 2.0 + SKY130 HD）}
\label{tab:pr8}
\footnotesize
\setlength{\tabcolsep}{2pt}
\begin{tabular}{lrrrrr}
\toprule
\textbf{电路} & \textbf{预布局基线} & \textbf{预布局修复} & \textbf{全局布线估计基线} & \textbf{全局布线估计修复} & $\Delta$ \\
\midrule
...
\bottomrule
\multicolumn{6}{p{0.92\columnwidth}}{\footnotesize 各电路的物理估计均采用与其预布局评估一致的映射网表及对应接受候选；s27 因面积过小采用 10\% 利用率布局规划；全局布线寄生估计后 $\Delta$ 为修复网表相对基线网表的 WNS 变化。}\\
\end{tabular}
\end{table}
...
实验结果表明，FAECO 在 ISCAS89、ITC-99 和 PicoRV32 上分别实现 8/8、18/19 和 2/3 个电路的严格 WNS 改善（b06 与基线持平）；29/30 个基线—修复对由顺序 SEC 完全证明，b17 剩余 1 个未证明点经 Liberty 检查确认为同函数尺寸替换。ISCAS89 20 轮收敛配置下，混合策略平均 WNS 改善为 1.07\,ns，高于纯 G 的 0.16\,ns。ITC-99 中仅 b06 未达严格改善，其未改善现象与多个相邻 \texttt{or4b$_1$} 在预布局理想线网下使 G 反向恶化、B 无低扇出网络可利用、R 候选在 SKY130 HD 库中不存在等因素相关，详见 \S\ref{sec:itc99}。

OpenROAD 布局及全局布线寄生估计后，8 个 ISCAS89 电路中，5 个保持改善、1 个持平，2 个退化不超过 0.02\,ns，多数电路在物理寄生估计后仍保持改善。
