...
\caption{FAECO 机制原理：局部扇入锥的特征中，边界/尺寸/验证/扇出写入加权最小割的节点代价(式(2))，关键路径由独立覆盖割 $C_c$ 与 $\lambda_c$ 的深度折扣及排序反馈处理；R 候选可用性驱动策略切换。图中“候选集合 A/B”为示意图标签，实际符号 $C_w/C_c$ 与表~2 一致。}
...
每类失败使对应权重增加 $1.0$：F1 增大边界惩罚 $\lambda_b$ 与扇出权重 $\lambda_f$，F2 增大边界惩罚 $\lambda_b$，F3 增大尺寸惩罚 $\lambda_s$，F4 增大关键路径深度折扣 $\lambda_c$，F5 增大验证代价惩罚 $\lambda_v$ 并将最大锥门数减半，F6 增大边界惩罚 $\lambda_b$，收紧候选边界并偏向较小局部以降低高线载候选再次出现的概率。
...
\midrule
F1 局部功能/结构失败 & Liberty 文件中的函数匹配或结构检查未通过 & 增加边界惩罚 $\lambda_b$ 与扇出权重 $\lambda_f$（抑制高扇出的不稳定等价点），降低此类节点被选作候选边界的概率 \\
...
F6 SPEF 复测失败 & 启用物理门控且理想线网增益 $>10\,$ps，但 SPEF 下 WNS 不改善 & 增加边界惩罚 $\lambda_b$，收紧候选边界并偏向较小局部，降低高线载候选再次出现的概率 \\
\bottomrule
\end{tabular}
\end{table}
