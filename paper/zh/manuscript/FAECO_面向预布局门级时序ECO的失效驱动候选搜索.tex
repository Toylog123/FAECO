...
F6 SPEF 复测失败 & 启用物理门控且理想线网增益 $>10\,$ps，但 SPEF 下 WNS 不改善 & 增加边界惩罚 $\lambda_b$ 与尺寸惩罚 $\lambda_s$，降低高扇出路径被选入候选集的概率 \\
\bottomrule
\end{tabular}
\end{table}
...
其中大规模电路 b18 和 b19 的进一步统计如下：在 0.5 ns 时钟周期下，b18 的 WNS 由 $-13.34$ ns 改善至 $-13.27$ ns（+$0.07$ ns），b19 由 $-17.44$ ns 改善至 $-16.00$ ns（+$1.44$ ns）；两者分别含 37.6 万和 75.5 万个门，经 93 次和 58 次候选级 STA 测量完成修复，测量次数保持在数十次量级。b18、b19 与上表 b14/b15/b17/b20/b21/b22 一起已包含在前述 19 个 ITC-99 电路中，此处仅作门数与测量成本补充说明。
...
三组数据集的最终基线网表与修复网表除 b17 一例外，均由独立顺序 SEC 完全证明（ISCAS89、ITC-99、PicoRV32 通过率分别为 8/8、18/19、3/3）；2/3 和 18/19 为 WNS 严格改善比例，SEC 完全证明比例分别为 8/8、18/19 和 3/3。b17 的修复网表在 SKY130 行为单元模型下经 Yosys \texttt{equiv\_simple} + \texttt{equiv\_induct} 验证，12812 个等价点已证明，剩余 1 个未证明点对应 nor4b$_1\!\to\!$nor4b$_2$ 的同函数尺寸替换（Liberty function 一致）；鉴于该点未被顺序 SEC 完全消解，本文将其单独报告。

\subsection{消融与基线对比}\label{sec:ablation}

本小节以禁用 F1--F6 反馈的纯 G、纯 B 和随机搜索作为固定权重基线，与启用 F4 反馈的 20 轮混合策略比较（表~\ref{tab:ablation}）。固定权重条件下，纯 B 与随机基线在不同电路上互有胜负，未表现出稳定优势。启用 F4 反馈的 20 轮混合策略平均 WNS 改善达到 1.07\,ns，并在所有电路上不劣于纯 B 与随机基线（8/8 不低于纯 G，7/8 严格高于纯 B，s27 上两者持平）。本表同时改变了候选空间（混合 R/G/B/JOINT）与反馈（F1--F6 启用），因此本节的对比并不单独隔离 F4 反馈的贡献；其意义在于表明，在当前候选空间下，混合策略与失效反馈的结合有助于将搜索集中于高收益候选。SPEF 复测阈值（5/10/20 ps）和权重扰动（$\pm 100\%$）均不改变结论，说明方法对参数设置具有一定稳健性。
...
\bottomrule
\multicolumn{5}{p{0.92\columnwidth}}{\footnotesize 数字为 WNS 改善量 (ns)，正值代表修复后 WNS 较基线提高；纯 G / 纯 B / 随机三列为\textbf{固定权重消融}（禁用 F1--F6 反馈），混合 20 列为启用 F4 反馈的 20 轮收敛配置（\texttt{--max-iterations 20}）。}\\
\end{tabular}
\end{table}
