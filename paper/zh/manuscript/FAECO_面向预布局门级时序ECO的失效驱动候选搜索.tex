...
\begin{enumerate}
  ...
  \item \textbf{功能—时序—物理分层验证}：将局部函数/结构筛选、理想线网 WNS 判定和简化 SPEF 复测串联起来，并以独立全网表 SEC 验证最终修复结果；30 个测试实例中 28 个产生实际网表修改，其中 27/28 个修改网表对由顺序 SEC 完全证明，b17 剩余 1 个未证明点经 Liberty 检查确认为同函数尺寸替换；2 个未改善实例（ITC-99 b06、PicoRV32 \texttt{picorv32\_regs}）未产生接受补丁，其输出网表与基线一致。
...
在 19 个可映射到 SKY130 HD 的 ITC-99 电路中，18 个电路的 WNS 取得严格改善，b06 与基线持平（$\Delta = 0\,\text{ns}$），19 个电路均无 WNS 回退。b17 在默认 60\,s 单候选验证时限下，所有候选均触发 F5 超时而被拒；为区分验证预算限制与搜索能力的差异，将该电路的单候选验证时限补充放宽至 180\,s，随后在 104 次候选级 STA 测量内取得 $+0.38\,$ns 改善（接受 G 候选 nor4b$_1\!\to\!$nor4b$_2$），其余 18 个电路仍采用 60\,s 默认配置。大规模电路 b14/b15/b17/b18/b19/b20/b21/b22 均取得严格 WNS 改善。b06 是 19 个电路中基线建立时间违例幅度最小者（WNS $= -0.06\,\text{ns}$），其关键路径包含多个相邻的 \texttt{or4b$_1$}，G 策略因增大输入电容拖累前级、B 策略无可缓解的低扇出网络、R 候选在 SKY130 HD 库中无等价族；在当前工艺库、候选空间和理想线网模型下，混合 R/G/B/JOINT 共 664 次候选级 STA 测量中 333 次与基线持平、0 次严格超过基线，表明 b06 在当前搜索空间中的进一步改善较为受限。实验在固定工艺库和时序约束下完成跨测试集评估。

策略分布以 JOINT 为主（12 个，b20/b21 的收益最大，分别为 $+1.98$ 和 $+2.16\,$ns）；19 个电路共进行 1012 次候选级 STA 测量，实际运行时间约 17 分钟。18 个严格改善电路的改善中位数为 $+0.18\,\text{ns}$、均值为 $+0.43\,\text{ns}$，后者由 b20/b21 大电路（基线 WNS $\le -13\,\text{ns}$）拖高，掩盖了中小电路的典型收益；论文同时报告中位数与均值以避免读者对均值产生过度乐观的解读。b06 的 $\Delta = 0$ 不影响 18/19 的改善比例，因此仍满足零回退统计。

其中大规模电路 b18 和 b19 的进一步统计如下：在 0.5 ns 时钟周期下，b18 的 WNS 由 $-13.34$ ns 改善至 $-13.27$ ns（+$0.07$ ns），b19 由 $-17.44$ ns 改善至 $-16.00$ ns（+$1.44$ ns）；两者分别含 37.6 万和 75.5 万个门，经 93 次和 58 次候选级 STA 测量完成修复，测量次数保持在数十次量级。b18、b19 未纳入上述 19 个电路的统一统计，在相同流程下作为大规模补充实例单独运行，此处补充其门数与测量成本。

\subsubsection{PicoRV32 异构族}

基于同一寄存器传输级（RTL）源的三个结构差异明显的子模块直接复用 ISCAS89 流程：PicoRV32 与其 PCPI 乘法协处理器 \texttt{picorv32\_pcpi\_mul} 分别经 R、G 获得 $+1.13$ 和 $+0.05\,$ns 的改善，存储密集子模块未改善。

三组数据集的 WNS 严格改善比例分别为 8/8、18/19 和 2/3。顺序 SEC 面向 30 个测试实例：28 个实例产生实际网表修改，其中 27/28 个修改网表对完全证明（三组分别为 8/8、17/18 和 2/2）；2 个未改善实例（b06、\texttt{picorv32\_regs}）未产生接受补丁，其输出网表与基线一致。b17 的修复网表在 SKY130 行为单元模型下经 Yosys \texttt{equiv\_simple} + \texttt{equiv\_induct} 验证，12812 个等价点已证明，剩余 1 个未证明点对应 nor4b$_1\!\to\!$nor4b$_2$ 的同函数尺寸替换（Liberty function 一致）；鉴于该点未被顺序 SEC 完全消解，本文将其单独报告。
...
\bottomrule
\multicolumn{5}{p{0.92\columnwidth}}{\footnotesize 数字为 WNS 改善量 (ns)，正值代表修复后 WNS 较基线提高；纯 G / 纯 B / 随机三列采用固定搜索权重，不启用 F1--F6 失效反馈，混合 20 轮列启用失效反馈。各方法均采用式~\eqref{eq:accept} 的候选接受准则，并使用相同的 20 轮上限与候选评估预算。随机列与混合列\textbf{共享 R/G/B/JOINT 候选类型与初始候选生成规则，每轮在同规则下生成候选池后随机化评估顺序}，3 种子（20260814、20260815 和 20260816）取均值。} \\
\end{tabular}
\end{table}
...
实验结果表明，FAECO 在 ISCAS89、ITC-99 和 PicoRV32 上分别实现 8/8、18/19 和 2/3 个电路的严格 WNS 改善（b06 与基线持平，仅指 WNS 改善，不等同于 timing closure）；30 个测试实例中 28 个产生实际网表修改，其中 27/28 个修改网表对由顺序 SEC 完全证明，b17 剩余 1 个未证明点经 Liberty 检查确认为同函数尺寸替换；2 个未改善实例（ITC-99 b06、PicoRV32 \texttt{picorv32\_regs}）未产生接受补丁，其输出网表与基线一致。ISCAS89 20 轮收敛配置下，混合策略平均 WNS 改善为 1.07\,ns，高于纯 G 的 0.16\,ns。ITC-99 中仅 b06 未达严格改善，其未改善现象与多个相邻 \texttt{or4b$_1$} 在预布局理想线网下使 G 反向恶化、B 无低扇出网络可利用、R 候选在 SKY130 HD 库中不存在等因素相关，详见 \S\ref{sec:itc99}。
