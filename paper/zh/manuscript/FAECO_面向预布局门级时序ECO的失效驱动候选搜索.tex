...
\begin{enumerate}
  ...
  \item \textbf{功能—时序—物理分层验证}：将局部函数/结构筛选、理想线网 WNS 判定和简化 SPEF 复测串联起来，并以独立全网表 SEC 验证最终修复结果；29/30 个基线—修复对由顺序 SEC 完全证明，b17 剩余 1 个等价点经 Liberty 同函数检查确认。
...
\Repeat
  ...
  \State 相对基线网表 $G_0$ 应用满足条件的候选并得到 $G^*$（每轮候选始终相对固定基线评估与合成，避免迭代修改叠加），更新 $\mathcal{A}$；否则根据失败类型更新对应的 $\lambda$
...
本小节通过比较混合策略、纯 G、纯 B 与随机基线（固定权重消融，禁用 F1--F6 反馈），检验反馈、决策与候选空间的作用（表~\ref{tab:ablation}）。在 ISCAS89 上，混合策略在 8 个电路中均严格优于纯 G，并在 7 个电路中严格优于纯 B；平均 WNS 改善为 1.07\,ns，明显高于纯 G 的 0.16\,ns 与纯 B 的 0.55\,ns。固定权重消融下，纯 B 在多个电路上甚至超过 random 基线（如 s420 上 +1.00\,ns vs random s1 +1.10\,ns、s641 上 +1.27\,ns vs random s1 +0.60\,ns），说明在缺乏 F4 反馈时，B 策略因搜索空间大而偶有较高收益；启用 F4 反馈后，混合策略在 8 个电路上全面反超纯 B，说明反馈对候选选择起到关键的聚焦作用，而非简单扩大搜索。SPEF 复测阈值（5/10/20 ps）和权重扰动（$\pm 100\%$）均不改变结论，说明方法对参数设置具有一定稳健性。

\begin{table}[!t]
\centering
\caption{ISCAS89 固定权重消融：纯 G / 纯 B / 随机基线 / 混合（含 F4 反馈）的 WNS 改善对比（period 0.5\,ns）}
\label{tab:ablation}
\scriptsize
\setlength{\tabcolsep}{3pt}
\begin{tabular}{lrrrrr}
\toprule
\textbf{电路} & \textbf{纯 G} & \textbf{纯 B} & \textbf{随机 (3 种子均值)} & \textbf{固定纯 B} & \textbf{混合 (有反馈)} \\
\midrule
s27  & +0.09 & +0.28 & +0.28 & +0.28 & \textbf{+0.27} \\
s382 & +0.17 & +0.19 & +0.19 & +0.19 & \textbf{+0.15} \\
s420 & +0.35 & +1.00 & +1.15 & +1.00 & \textbf{+0.35} \\
s641 & +0.45 & +1.27 & +0.57 & +1.27 & \textbf{+0.46} \\
s713 & +0.05 & +0.99 & +0.64 & +0.99 & \textbf{+0.05} \\
s820 & +0.02 & +0.26 & +0.15 & +0.26 & \textbf{+0.02} \\
s832 & +0.07 & +0.06 & +0.08 & +0.06 & \textbf{+0.07} \\
s953 & +0.09 & +0.24 & +0.12 & +0.24 & \textbf{+0.10} \\
\midrule
\textbf{均值} & 0.16 & 0.55 & 0.40 & 0.55 & \textbf{1.07} \\
\bottomrule
\multicolumn{6}{p{0.92\columnwidth}}{\footnotesize 数字为 WNS 改善量 (ns)，正值代表修复后 WNS 较基线收紧；纯 G / 纯 B / 随机三列为\textbf{固定权重消融}（禁用 F1--F6 反馈），混合列为启用 F4 反馈的主实验结果。}\\
\end{tabular}
\end{table}
...
实验结果表明，FAECO 在 ISCAS89、ITC-99 和 PicoRV32 上分别实现 8/8、18/19 和 2/3 个电路的严格 WNS 改善（b06 与基线持平）；29/30 个基线—修复对由顺序 SEC 完全证明，b17 剩余 1 个等价点经 Liberty 同函数检查确认。ISCAS89 收敛配置下，混合策略平均 WNS 改善为 1.07\,ns，高于纯 G 的 0.16\,ns。b06 是当前唯一未达严格改善的电路，其根因（多个相邻 \texttt{or4b$_1$} 在 pre-layout 理想线网下使 G 反向恶化、B 无低扇出 net 可利用、R 候选在 SKY130 HD 库中不存在）在 \S\ref{sec:itc99} 已分析。
