...
\Repeat
  ...
  \State 触发 F1--F6 的失败候选根据对应失败类型更新搜索权重；因 10\,ps 阈值直接过滤的候选不参与失效反馈
\Until{无时序违例、候选耗尽或达到停止条件}
\State \Return $G^*,\ \mathcal{A}$
\end{algorithmic}
\end{algorithm}

算法~\ref{alg:main} 给出从违例端点提取扇入锥、生成候选、时序测量到反馈重搜索的流程。候选按轮提交：每轮接受的候选经验证后成为下一轮的评估基准，因此多轮配置下修复网表由接受的补丁逐轮累积形成；效率优先配置（\S\ref{sec:eff_prio}）仅执行单轮，不产生累积叠加。候选接受谓词和验证层次在 \S\ref{sec:validation} 中统一定义。
...
本小节比较候选空间与候选排序两类设置（表~\ref{tab:ablation}）：纯 G、纯 B 采用单策略候选空间；随机列与混合 20 轮列采用混合候选空间，前者随机化候选评估顺序并取种子 1/2/3 的均值，后者按割代价与路径覆盖排序。各方法均采用式~\eqref{eq:accept} 作为候选接受准则，并采用相同的 20 轮上限和候选评估预算。需要说明的是，本组各列由同一固定权重运行器产生，该运行器不包含 F1--F6 权重重整路径，因此本表对比的是候选空间与候选排序的差异，不构成失效反馈的开关对照；失效反馈的独立贡献需另设对照实验量化。在混合候选空间下，纯 B 与随机基线在不同电路上互有胜负，未表现出稳定优势。20 轮混合策略平均 WNS 改善达到 1.07\,ns，在 8/8 个电路上严格高于纯 G，并在 7/8 个电路上严格高于纯 B 和随机基线，s27 上与后二者持平。
...
\bottomrule
\multicolumn{5}{p{0.92\columnwidth}}{\footnotesize 数字为 WNS 改善量 (ns)，正值代表修复后 WNS 较基线提高；纯 G 与纯 B 列分别只生成 G / B 候选，随机列与混合 20 轮列在混合候选空间上生成候选（混合列另含 JOINT 组合候选），随机列在每轮生成候选池后随机化评估顺序，随机种子 1、2、3 三组取均值。各列均使用固定搜索权重、不启用 F1--F6 权重重整，并采用式~\eqref{eq:accept} 的候选接受准则与相同的 20 轮上限和候选评估预算；因此本表量化候选空间与候选排序的贡献，失效反馈的独立贡献不在本表范围内。} \\
\end{tabular}
\end{table}
