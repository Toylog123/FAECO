...
\Repeat
  ...
  \State 相对基线网表 $G_0$ 应用满足条件的候选并得到 $G^*$（每轮候选始终相对固定基线评估与合成，避免迭代修改叠加），更新 $\mathcal{A}$；$\mathcal{A}$ 中可累计多轮接受的多个候选，但本算法每轮保留当前 WNS 改善最大的单个 patch 作为 $G^*$，其余候选仅作失败反馈使用；否则根据失败类型更新对应的 $\lambda$
...
\textbf{实验配置}：所有电路采用相同主流程和默认参数，比较效率优先和收敛两种配置；b17 的验证预算例外见 \S\ref{sec:itc99}。候选以 WNS 严格改善为接受条件，理想增益超过 10 ps 后进行 SPEF 复测；完整脚本和参数见实验产物。
...
三组数据集的最终基线网表与修复网表除 b17 一例外，均由独立顺序 SEC 完全证明（ISCAS89、ITC-99、PicoRV32 通过率分别为 8/8、18/19、3/3）；2/3 和 18/19 为 WNS 严格改善比例，SEC 完全证明比例分别为 8/8、18/19 和 3/3。b17 的修复网表在 SKY130 行为单元模型下经 Yosys \texttt{equiv\_simple} + \texttt{equiv\_induct} 验证，12812 个等价点已证明，剩余 1 个未证明点对应 nor4b$_1\!\to\!$nor4b$_2$ 输出 wire 的同函数尺寸替换（Liberty function 一致，但 Yosys \texttt{find\_same\_wires} 在重命名 cell 输出 wire 上给出假阴性），本文将其单独报告而不计作 SEC 完全消解。

\subsection{消融与基线对比}\label{sec:ablation}

本小节通过比较混合策略、纯 G、纯 B 与随机基线（固定权重消融，禁用 F1--F6 反馈），检验反馈、决策与候选空间的作用（表~\ref{tab:ablation}）。在 ISCAS89 20 轮收敛配置下，混合策略在 8 个电路中均严格优于纯 G（8/8），并在 7 个电路中严格优于纯 B（7/8，s27 上两者持平）；平均 WNS 改善为 1.07\,ns，明显高于纯 G 的 0.16\,ns 与纯 B 的 0.54\,ns。固定权重消融下，纯 B 多个电路上与随机基线 3 种子均值互有胜负（如 s420 上 +1.00\,ns vs random 3 种子均值 +1.15\,ns、s641 上 +1.27\,ns vs random 3 种子均值 +0.57\,ns），说明在缺乏 F4 反馈时 B 与 random 难以稳定领先；启用 F4 反馈后，混合策略在 8 个电路上稳定反超纯 B 与随机基线，说明反馈对候选选择起到关键的聚焦作用，而非简单扩大搜索。SPEF 复测阈值（5/10/20 ps）和权重扰动（$\pm 100\%$）均不改变结论，说明方法对参数设置具有一定稳健性。

\begin{table}[!t]
\centering
\caption{ISCAS89 固定权重基线与失效反馈混合策略的 WNS 改善对比（period 0.5\,ns）}
\label{tab:ablation}
\scriptsize
...
\bottomrule
\multicolumn{5}{p{0.92\columnwidth}}{\footnotesize 数字为 WNS 改善量 (ns)，正值代表修复后 WNS 较基线收紧；纯 G / 纯 B / 随机三列为\textbf{固定权重消融}（禁用 F1--F6 反馈），混合 20 列为启用 F4 反馈的 20 轮收敛配置（``--max-iterations 20``）。}\\
\end{tabular}
\end{table}
...
\textbf{迭代式门控}：在 40\,$\mu$m/unit 估计线载下（unit 表示一次扇出后沿拓扑/曼哈顿距离累加的相对单位长度，配合简化 RC 换算用于预布局线载估计），s382 的 50 个与 b18 的 6 个候选均未通过 SPEF 复测，并按 F6 更新权重；门控依据 SPEF 结果筛除物理无效候选并更新搜索权重。简化 SPEF 的生成规则与单位换算见实验产物。
...
实验结果表明，FAECO 在 ISCAS89、ITC-99 和 PicoRV32 上分别实现 8/8、18/19 和 2/3 个电路的严格 WNS 改善（b06 与基线持平）；29/30 个基线—修复对由顺序 SEC 完全证明，b17 剩余 1 个未证明点经 Liberty 检查确认为同函数尺寸替换。ISCAS89 20 轮收敛配置下，混合策略平均 WNS 改善为 1.07\,ns，高于纯 G 的 0.16\,ns。b06 是当前唯一未达严格改善的电路，其根因（多个相邻 \texttt{or4b$_1$} 在 pre-layout 理想线网下使 G 反向恶化、B 无低扇出 net 可利用、R 候选在 SKY130 HD 库中不存在）在 \S\ref{sec:itc99} 已分析。
