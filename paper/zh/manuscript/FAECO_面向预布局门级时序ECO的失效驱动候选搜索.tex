...
\begin{enumerate}
  ...
  \item \textbf{跨测试集与真实流程验证}：在 ISCAS89、ITC-99 和 PicoRV32 上分别实现 8/8、18/19 和 2/3 个电路的严格 WNS 改善；ISCAS89 20 轮收敛配置下混合策略平均 WNS 改善为 1.07\,ns，另在效率优先配置对应的真实布局布线验证中 5/8 个 ISCAS89 电路保持改善。
\end{enumerate}
...
给定映射网表 $G'$、目标周期 $T$、时序约束和工艺库 $\mathcal{L}$，FAECO 在违例端点的扇入锥上搜索局部候选 $p$，得到修复网表 $G^*$。主实验以 $\Delta_{\mathrm{WNS}}(p)>0$ 作为理想线网时序接受谓词，并要求修复前后网表保持功能等价；启用物理门控时，还需满足 \S\ref{sec:failures} 所述 SPEF 复测条件。可选的 TNS 感知配置仅在 WNS 持平且 TNS 改善时接受候选。四类局部变换为：R（布尔函数等价替换）、G（驱动尺寸调整）、B（恒等缓冲器插入）和 JOINT（联合枚举）。顺序实验不修改 D 触发器、时钟或复位单元。
...
\Repeat
  ...
  \State 若候选满足式~\eqref{eq:accept}，并在启用物理门控时通过简化 SPEF 复测，则将其加入 $\mathcal{A}$；若其 $\Delta_{\mathrm{WNS}}$ 优于当前最佳值，则相对基线网表 $G_0$ 应用该候选并更新 $G^*$。未通过接受判据的候选依据 F1--F6 失败类型更新对应权重。各轮候选均相对固定基线 $G_0$ 评估，不叠加多个 patch。
...
\textbf{实验配置}：所有电路采用相同主流程和默认参数，比较效率优先和收敛两种配置；b17 的验证预算例外见 \S\ref{sec:itc99}。候选以 WNS 严格改善为接受条件，理想增益超过 10 ps 后进行 SPEF 复测；完整脚本和参数随补充材料一同发布。
...
本节物理验证采用 \S\ref{sec:eff_prio} 效率优先配置得到的接受候选，与 \S\ref{sec:ablation} 的 20 轮收敛配置分开统计。在真实布局布线（placement and routing，P\&R）后，8 个 ISCAS89 电路中有 5 个电路保持时序改善，1 个持平，2 个轻微恶化但不超过 0.02 ns（表~\ref{tab:pr8}）。流程使用 OpenROAD 2.0、SKY130 HD 和 0.5 ns 时钟约束，并在全局布线估计寄生后进行时序对比；SPEF 负载扫描和迭代式门控用于量化线载影响，并将 F6 复测失败接入权重更新。
