...
\begin{enumerate}
  ...
  \item \textbf{跨测试集与真实流程验证}：在 ISCAS89、ITC-99 和 PicoRV32 上分别实现 8/8、18/19 和 2/3 个电路的严格 WNS 改善；ISCAS89 收敛配置下混合策略平均 WNS 改善为 1.07\,ns，真实布局布线后 8 个 ISCAS89 电路中有 5 个保持改善。
\end{enumerate}
...
在 19 个可映射到 SKY130 HD 的 ITC-99 电路中，18 个电路的 WNS 取得严格改善，b06 与基线持平（$\Delta = 0\,\text{ns}$），19 个电路均无 WNS 回退；大规模电路 b14/b15/b17/b20/b21/b22 均成功。b17 在默认 60\,s 单候选验证时限下，所有候选均触发 F5 超时而被拒；为区分验证预算限制与搜索能力的差异，将该电路的单候选验证时限补充放宽至 180\,s，随后在 104 次候选级 STA 测量内取得 $+0.38\,$ns 改善（接受 G 候选 nor4b$_1\!\to\!$nor4b$_2$），其余 18 个电路仍采用 60\,s 默认配置。b06 是 19 个电路中基线 setup 违例幅度最小者（WNS $= -0.06\,\text{ns}$），其关键路径包含多个相邻的 \texttt{or4b$_1$}，G 策略因增大输入电容拖累前级、B 策略无可缓解的低扇出 net、R 候选在 SKY130 HD 库中无等价族；在当前工艺库、候选空间和理想线网模型下，混合 R/G/B/JOINT 共 664 次候选级 STA 测量中 333 次与基线持平、0 次严格超过基线，表明 b06 在当前搜索空间中的进一步改善较为受限。实验在固定工艺库和时序约束下完成跨测试集评估。

策略分布以 JOINT 为主（12 个，b20/b21 的收益最大，分别为 $+1.98$ 和 $+2.16\,$ns）；19 个电路共进行 1012 次候选级 STA 测量，实际运行时间约 17 分钟。所有改善的中位数为 $+0.18\,\text{ns}$、均值为 $+0.43\,\text{ns}$，后者由 b20/b21 大电路（baseline WNS $\le -13\,\text{ns}$）拖高，掩盖了中小电路的典型收益；论文同时报告中位数与均值以避免读者对均值产生过度乐观的解读。b06 的 $\Delta = 0$ 不影响 18/19 的改善比例，因此仍满足零回退统计。
...
本小节以不启用 F1--F6 反馈的纯 G、纯 B 和随机搜索作为固定权重基线，与启用 failure-aware 早停的 20 轮混合策略比较（表~\ref{tab:ablation}）。固定权重条件下，纯 B 与随机基线在不同电路上互有胜负，未表现出稳定优势。failure-aware 早停的 20 轮混合策略平均 WNS 改善达到 1.07\,ns，并在所有电路上不劣于固定权重基线（8/8 不低于纯 G，7/8 严格高于纯 B，s27 上两者持平）。本表同时改变了候选空间（混合 R/G/B/JOINT）与反馈配置，因此不用于单独量化 F1--F6 反馈的独立贡献；其意义在于表明，混合候选空间与 failure-aware 早停的组合能够获得更高的搜索收益。SPEF 复测阈值（5/10/20 ps）和权重扰动（$\pm 100\%$）均不改变结论，说明方法对参数设置具有一定稳健性。
...
\bottomrule
\multicolumn{5}{p{0.92\columnwidth}}{\footnotesize 数字为 WNS 改善量 (ns)，正值代表修复后 WNS 较基线提高；纯 G / 纯 B / 随机三列为\textbf{固定权重基线}（不启用 F1--F6 反馈），混合 20 列为启用 failure-aware 早停的 20 轮配置（\texttt{--max-iterations 20}）。各对照采用相同的 20 轮上限与候选评估预算。}\\
\end{tabular}
\end{table}
...
本节物理验证采用 \S\ref{sec:ood} 效率优先配置得到的接受候选，与 \S\ref{sec:ablation} 的 20 轮收敛配置分开统计。在真实布局布线（placement and routing，P\&R）后，8 个 ISCAS89 电路中有 5 个电路保持时序改善，1 个持平，2 个轻微恶化但不超过 0.02 ns（表~\ref{tab:pr8}）。流程使用 OpenROAD 2.0、SKY130 HD 和 0.5 ns 时钟约束，并在全局布线估计寄生后进行时序对比；SPEF 负载扫描和迭代式门控用于量化线载影响，并将 F6 复测失败接入权重更新。
