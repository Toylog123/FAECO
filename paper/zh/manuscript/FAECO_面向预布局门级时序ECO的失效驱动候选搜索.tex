...
\Repeat
  ...
  \State 若候选满足接受谓词，则将其加入 $\mathcal{A}$；若其 $\Delta_{\mathrm{WNS}}$ 优于当前最佳值，则相对基线网表 $G_0$ 应用该候选并更新 $G^*$。未满足接受谓词的候选依据 F1--F6 失败类型更新对应权重。各轮候选均相对固定基线 $G_0$ 评估，不叠加多个 patch。
...
在 19 个可映射到 SKY130 HD 的 ITC-99 电路中，18 个电路的 WNS 取得严格改善，b06 与基线持平（$\Delta = 0\,\text{ns}$），19 个电路均无 WNS 回退；大规模电路 b14/b15/b17/b20/b21/b22 均成功。b17 在默认 60\,s 单候选验证时限下，所有候选均触发 F5 超时而被拒；为区分验证预算限制与搜索能力的差异，将该电路的单候选验证时限补充放宽至 180\,s，随后在 104 次候选级 STA 测量内取得 $+0.38\,$ns 改善（接受 G 候选 nor4b$_1\!\to\!$nor4b$_2$），其余 18 个电路仍采用 60\,s 默认配置。大规模电路 b14/b15/b17/b18/b19/b20/b21/b22 均成功。b06 是 19 个电路中基线 setup 违例幅度最小者（WNS $= -0.06\,\text{ns}$），其关键路径包含多个相邻的 \texttt{or4b$_1$}，G 策略因增大输入电容拖累前级、B 策略无可缓解的低扇出 net、R 候选在 SKY130 HD 库中无等价族；在当前工艺库、候选空间和理想线网模型下，混合 R/G/B/JOINT 共 664 次候选级 STA 测量中 333 次与基线持平、0 次严格超过基线，表明 b06 在当前搜索空间中的进一步改善较为受限。实验在固定工艺库和时序约束下完成跨测试集评估。

策略分布以 JOINT 为主（12 个，b20/b21 的收益最大，分别为 $+1.98$ 和 $+2.16\,$ns）；19 个电路共进行 1012 次候选级 STA 测量，实际运行时间约 17 分钟。18 个严格改善电路的改善中位数为 $+0.18\,\text{ns}$、均值为 $+0.43\,\text{ns}$，后者由 b20/b21 大电路（baseline WNS $\le -13\,\text{ns}$）拖高，掩盖了中小电路的典型收益；论文同时报告中位数与均值以避免读者对均值产生过度乐观的解读。b06 的 $\Delta = 0$ 不影响 18/19 的改善比例，因此仍满足零回退统计。

其中大规模电路 b18 和 b19 的进一步统计如下：在 0.5 ns 时钟周期下，b18 的 WNS 由 $-13.34$ ns 改善至 $-13.27$ ns（+$0.07$ ns），b19 由 $-17.44$ ns 改善至 $-16.00$ ns（+$1.44$ ns）；两者分别含 37.6 万和 75.5 万个门，经 93 次和 58 次候选级 STA 测量完成修复，测量次数保持在数十次量级。b18、b19 与前述 b14/b15/b17/b20/b21/b22 一样，均已包含在前述 19 个 ITC-99 电路统计中，此处仅作门数与测量成本补充说明。
...
本小节以不启用 failure-aware 接受控制（按式~\eqref{eq:accept}比较 WNS）的纯 G、纯 B 和随机搜索作为固定权重基线，与启用 failure-aware 接受控制的 20 轮混合策略比较（表~\ref{tab:ablation}）。固定权重条件下，纯 B 与随机基线在不同电路上互有胜负，未表现出稳定优势。启用 failure-aware 接受控制的 20 轮混合策略平均 WNS 改善达到 1.07\,ns，并在所有电路上不劣于固定权重基线（8/8 不低于纯 G，7/8 严格高于纯 B，s27 上两者持平）。本表同时改变了候选空间（混合 R/G/B/JOINT）与接受控制，因此不用于单独量化 failure-aware 控制相对其他策略的贡献；其意义在于表明，混合候选空间与 failure-aware 接受控制的组合能够获得更高的搜索收益。SPEF 复测阈值（5/10/20 ps）和权重扰动（$\pm 100\%$）均不改变结论，说明方法对参数设置具有一定稳健性。\footnote{本节 acceptance control 指式~\eqref{eq:accept} 的 failure-aware 比较，与 \S\ref{sec:failures} 中 F1--F6 失败类型驱动的外层权重更新为正交机制；后者由 \texttt{run\_outerloop\_real\_wns.py} 启用。}
...
\bottomrule
\multicolumn{5}{p{0.92\columnwidth}}{\footnotesize 数字为 WNS 改善量 (ns)，正值代表修复后 WNS 较基线提高；纯 G / 纯 B / 随机三列为\textbf{固定权重基线}（不启用 failure-aware 接受控制），混合 20 列为启用 failure-aware 接受控制的 20 轮配置（\texttt{--max-iterations 20}）。各对照采用相同的 20 轮上限与候选评估预算。随机列在相同候选池内随机候选评估顺序，3 种子（20260814 / 20260814+1 / 20260814+2）取均值。}\\
\end{tabular}
\end{table}
...
实验结果表明，FAECO 在 ISCAS89、ITC-99 和 PicoRV32 上分别实现 8/8、18/19 和 2/3 个电路的严格 WNS 改善（b06 与基线持平）；29/30 个基线—修复对由顺序 SEC 完全证明，b17 剩余 1 个未证明点经 Liberty 检查确认为同函数尺寸替换。ISCAS89 20 轮收敛配置下，混合策略平均 WNS 改善为 1.07\,ns，高于纯 G 的 0.16\,ns。b06 是当前唯一未达严格改善的电路，其未改善现象与多个相邻 \texttt{or4b$_1$} 在 pre-layout 理想线网下使 G 反向恶化、B 无低扇出 net 可利用、R 候选在 SKY130 HD 库中不存在等因素相关，详见 \S\ref{sec:itc99}。
