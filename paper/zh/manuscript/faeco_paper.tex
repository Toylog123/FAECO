...
结论（诚实、负面导向）：**在理想网络下有效的修复（s382 +0.01、b18 +0.13）在加入寄生 RC 后改善归零**——pre-layout 修复不保证 post-layout 有效，这正是本方法当前最重要的边界，也量化了物理感知 ECO 的必要性。该 SPEF 为简化估计（lumped RC、非真实 P\&R 版图），完整 OpenROAD + PDK LEF/DEF 流程未接入（见 limitation L31-01），作为方法边界而非成熟物理验证。
...
FAECO 把“重综合辅助 patch replacement”形式化为 failure-aware 的切割、替换与验证流水线：外环以 F1--F5 失败分类驱动确定性权重调整并在公开基准上端到端跑通，内环以 R/G/B 混合修复经 OpenSTA 实测逐候选接受，在 ISCAS89 顺序电路上 8/8 改善 WNS（s382 收敛至 MET），并进一步在 ITC-99 基准族上实现 19 个可映射电路 17/19 真实 WNS 改善（含 6 个大电路）。全部实验基于 EPFL v2025.1 / ISCAS89 / ITC-99 公开基准与 SKY130 HD 标准单元库，基于 Yosys + ABC + OpenSTA 开源工具链完成，产物可复现。

\textbf{当前 limitation（透明声明）}：(1) 内环验证为 pre-layout ideal-net，post-layout 长线负载下 G/B 的相对贡献可能变化；我们已用 OpenSTA 的 SPEF 读取能力补充寄生参数感知验证（见 \S\ref{sec:parasitic}），但完整 P\&R 版图验证（OpenROAD + PDK LEF/DEF）尚未接入；(2) 外环真实 WNS 闭环已扩展到 19 个 ITC-99 电路（17/19 改善）并含 6 个大电路，b18/b19 已通过 WSL2 64 位 Yosys 0.33 补全（b18 经 JOINT 联合修复 -0.69$\rightarrow$-0.56，b19 在 95% 现实周期下由 R 修复达成 MET），但多 corner 与多工艺库仍未验证；(3) Z3 candidate/boundary 形式回验的 8-case 端到端受 mapped.v 门级实例化限制，wrapper 单元测试层面已验证。

\textbf{未来工作}：(1) 将 64 位 Yosys 验证扩展到 b18/b19 的多 corner 与多工艺库，验证权重细化与决策层表的跨工艺迁移；(2) N31-03 ORFS techmap library 消除占位单元、修复 Stage B CEC；(3) Z3 AIG$\rightarrow$SMT 路径处理 mapped.v 门级实例化；(4) 结合关键路径覆盖候选与多策略内环，量化“反馈学习 vs 宽度探索”在更大搜索空间的效率边界。
