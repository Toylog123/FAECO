...
\begin{table}[!t]
\centering
\caption{在线自适应决策层（无先验冷启动）vs 静态优先级表（period 0.5ns，beam=1 + early-stop）}
...
\begin{table}[!t]
\centering
\caption{ITC-99 跨基准泛化：19 个可映射电路真实 WNS 修复（period 0.5ns，外环反馈+决策层+beam=1+early-stop）}
\label{tab:itc99}
\scriptsize
\setlength{\tabcolsep}{3pt}
\begin{tabular}{lrrrr}
...
\begin{itemize}
  \item \textbf{s382 全修复复跑}：真 0.67 映射下 baseline WNS $-0.98$，外环多轮迭代（候选 + 权重细化 + random\_cut）接受关键路径覆盖修复，final WNS \textbf{$-0.85$}——优于旧工具链（0.33 下 $-0.92$）。
  ...
  \item \textbf{全量回归}：252 项测试全绿（含双目标联合割、物理门控、分治切割新增测试）。
\end{itemize}
