...
随后接入 WSL2 的 64 位 Yosys 0.33，将此前因 32 位内存限制无法映射的 b18/b19 大电路补全：b18（37.6 万门）0.5ns 协议下基线 $-13.34\,$ns、修复 $-13.27$；b19（75.5 万门）基线 $-17.44$、修复 $-16.0$（接受 R clkinv$_1$$\rightarrow$bufinv$_8$，单候选改善 1.44 ns）。按 95\% 原始关键路径设置现实周期（b18 13.15 ns、b19 17.04 ns）后，WNS 落在实际 ECO 区间：b18 $-0.69\rightarrow-0.62$（G maj3$_1$$\rightarrow_2$），b19 $-0.90\rightarrow+0.54$ 达成时序收敛（MET、TNS 清零，R 候选）。b18 改善较小的边界（关键路径 36/38 为 XOR/MAJ/XNOR 复杂门，库中无等价替换候选，G 仅微调）如实记录。
...
\textbf{当前 limitation（透明声明）}：(1) 内环验证为 pre-layout ideal-net，post-layout 长线负载下 G/B 的相对贡献可能变化；(2) 外环真实 WNS 闭环已扩展到 19 个 ITC-99 电路（17/19 改善）并含 6 个大电路，b18/b19 已通过 WSL2 64 位 Yosys 0.33 补全（b19 在 95% 现实周期下由 R 修复达成 MET），但多 corner 与多工艺库仍未验证；(3) Z3 candidate/boundary 形式回验的 8-case 端到端受 mapped.v 门级实例化限制，wrapper 单元测试层面已验证。
