...
\textbf{当前 limitation（透明声明）}：(1) 内环验证为 pre-layout ideal-net，post-layout 长线负载下 G/B 的相对贡献可能变化；(2) 外环真实 WNS 闭环已扩展到 19 个 ITC-99 电路（17/19 改善）并含 6 个大电路，但 b18/b19（23--46 万门）超出 Yosys 0.9 32 位映射能力、多 corner 仍未验证；(3) Z3 candidate/boundary 形式回验的 8-case 端到端受 mapped.v 门级实例化限制，wrapper 单元测试层面已验证。

\textbf{未来工作}：(1) 接入 64 位 Yosys/OpenROAD 补齐 b18/b19 大电路，并扩展到多 corner/多工艺库验证权重细化与决策层表的跨工艺迁移；(2) N31-03 ORFS techmap library 消除占位单元、修复 Stage B CEC；(3) Z3 AIG$\rightarrow$SMT 路径处理 mapped.v 门级实例化；(4) 结合关键路径覆盖候选与多策略内环，量化“反馈学习 vs 宽度探索”在更大搜索空间的效率边界。
