...
\begin{table}[!t]
\centering
\caption{外环真实 WNS：beam$\times$反馈消融（4 电路，period 0.5ns）}
...
\midrule
s27  & $-0.28$ & 第 2 轮 $-0.21$（8--11 STA） & 8 轮失败（24--48 STA） & G（nor3b\_2） \\
s382 & $-0.94$ & 第 2 轮 $-0.92$（12--14 STA） & 8 轮失败（16--32 STA） & R（or2\_4） \\
s420 & $-1.78$ & 第 2 轮 $-1.51$（12--13 STA） & 8 轮失败（8--16 STA） & G（nor4\_4） \\
s641 & $-1.86$ & 第 1 轮 $-1.85$（3 STA） & 第 1 轮 $-1.85$（3 STA） & G（o21ai\_1） \\
\bottomrule
\end{tabular}
\end{table}

需“学习”的 3/4 电路：反馈 ON 把找到修复所需 beam 从 $\geq 4$--$8$ 压到 $\leq 2$，STA 调用省 $44\%$--$89\%$（s27 11 vs 46、s382 14 vs 25、s420 13 vs 122）；s641 首候选即成功（加权 min-cut 首候选已覆盖瓶颈门），反馈与宽度均无关。这是 FAECO“失败反馈学习替代蛮力宽度搜索”的首次真实时序量化证据。
