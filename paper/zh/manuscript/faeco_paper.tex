...
决策层（\texttt{--strategy-priorities auto}）在 s832 上不仅省 15\% STA 调用，还找到略优解；leave-one-out 跨电路迁移 top-2 预测命中率 94.3\%--98.2\%，证明策略选择规律跨 ISCAS89 电路高度稳定。
...
L31-04 & failure\_recovery 曾为 single-iteration proxy & 真实重综合网表 + 外环成功路径打通 + 权重进 min-cut；剩余：evaluator 全局判定解耦、OpenSTA WNS 接入 \\
补充 & METH-12 candidate-specific timing gain 为 Stage A proxy & 内环已接 OpenSTA per-candidate 实测；外环 WNS 驱动接入为后续项 \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{当前 limitation（透明声明）}：(1) 外环 evaluator 成功条件仍与 cut 位置解耦（全局 reduction 判定），WNS 驱动的成功标准真实接入 OpenSTA 为后续工程项；(2) 内环验证为 pre-layout ideal-net，post-layout 长线负载下 G/B 的相对贡献可能变化；(3) Z3 candidate/boundary 形式回验的 8-case 端到端受 mapped.v 门级实例化限制，wrapper 单元测试层面已验证。

\textbf{未来工作}：(1) 外环 WNS 成功标准真实接入 OpenSTA，形成 patch 级 recovery 的完整闭环；(2) N31-03 ORFS techmap library 消除占位单元、修复 Stage B CEC；(3) Z3 AIG$\rightarrow$SMT 路径处理 mapped.v 门级实例化；(4) 将决策层策略表扩展到更多工艺库与 corner，验证跨工艺迁移；(5) 更大规模顺序基准（ITC-99）上验证混合修复扩展性。
