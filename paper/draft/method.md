...
**复核修正（2026-08-04）**：外环端到端复核发现实现层根因，早前「F3 被权重反馈消除」表述有误——实测 c432/c499/c880 每轮只触发 F4，F3 从不触发，F3 消除不可复现。两个结构性根因：(1) F1 默认用结构签名等价（结构相同才 pass），与 F4 的 logic-level reduction >= 1（要求结构不同）互斥，成功路径在构造上不可达；(2) 候选排序 cost 单调加性、1-gate critical-path-only cut 恒最便宜，只有 F3 的 size_penalty 会额外加入候选，F1/F2/F4 权重不进入排序 cost，反馈在实现层面惰性。已为 run_multi_iteration_case 注入 functional equivalence checker（3 项回归测试，成功路径可达）。**权重惰性已修复（2026-08-04 第二轮）**：build_weighted_cut_graph 重写为 F1-F5 全权重进入节点成本（boundary_penalty 抬高边界浅层 gate、size_penalty 按 fanin-cone 二次项压缩、critical_coverage_reward 按逻辑深度降关键 gate 成本、equivalence_stability_reward 惩罚高扇出、verification_cost_penalty 按 cone 规模放大），weighted_cut_candidates 真正求解加权 s-t min-cut（tests/test_weighted_cut_feedback.py 6 项：size_penalty=5 时 cut 从 root 翻转到浅层 gate、critical_reward=5 选最深 gate）。剩余：OpenSTA WNS 成功标准接入为后续工程项。
