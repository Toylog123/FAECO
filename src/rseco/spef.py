...
def estimate_net_rc(net: MappedNet, *, unit_len_um: float = 40.0) -> tuple[float, float]:
    length = unit_len_um * max(1.0, (net.fanout ** 0.5))
    r = 0.09 * length
...
def build_spef(netlist: MappedNetlist, *, unit_len_um: float = 40.0) -> str:
    lines: list[str] = []
    ...
    for net in netlist.nets.values():
        if not net.pins:
            continue
        r_ohm, c_pf = estimate_net_rc(net, unit_len_um=unit_len_um)
...
def write_spef(path: str | Path, netlist: MappedNetlist, *, unit_len_um: float = 40.0) -> Path:
    out = Path(path)
    out.write_text(build_spef(netlist, unit_len_um=unit_len_um), encoding="utf-8")
    return out
