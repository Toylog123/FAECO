...
| OI-012 | **论文 §4.4 表 6 的"失效反馈开/关"对照在运行器层面不成立** | 2026-09-23 | **待修正** | §4.4 称"以**禁用 F1–F6 失效反馈**、保持权重固定的纯 G/纯 B/随机为基线，与**启用失效反馈**的 20 轮混合策略比较"。但表 6 四列的运行器均为 `code/scripts/run_hybrid_repair.py`（混合列由 `20260807_multiround_8c_067/run_all.bat` 证实），该脚本**完全不 import 且不使用** `refine_weights`/`RefinementWeights`/`enable_feedback`（仅 import gate_sizing/buffer_insertion/logic_rewrite/netlist_audit/strategy_selector）→ **四列都不含 F1–F6 反馈**，该表实际对比的是**候选空间 + 排序启发式**，不是反馈开/关。论文自陈的"同时改变候选空间和反馈配置"前提亦不成立（实际混杂为候选空间 + 排序）。**附：表 6 三列基线所用的 `20260826_ablation_pureG/pureB/random_seed{1,2,3}` 目录内无任何 .bat/.sh/.md/.log（顶层运行脚本 0 个），运行命令未归档**；仅被取代的 `20260807_pureG_20round_067`/`20260807_pureB_20round_067` 保留了 `run_all.bat`（如 pureG 为 `--circuit … --period 0.5 --rounds 20 --only-strategy G --workers 2`）。故论文所引那批消融的"是否禁用反馈/是否固定权重"**既无代码路径可证、也无调用记录可查** | `code/scripts/run_hybrid_repair.py`（imports 与函数体 grep）、`experiments/20260807_multiround_8c_067/run_all.bat`、`experiments/20260826_ablation_*/hybrid_result.json`、各目录顶层文件清点（0826 家族 0 个 vs 0807 家族 9–10 个） | 改写 §4.4：表 6 定位为"候选空间/排序消融"；把"失效反馈独立贡献"交由重做的三臂对照（见 `planning/FAECO_V2_DESIGN_PLAN_20260923.md` §2）承担——三臂必须跑在同一运行器、同一轮数/预算/初始权重，并在结果 JSON 中记录 `enable_feedback`/`random_order`/`init_weights`（当前均未记录），**并在实验目录归档运行命令** | 用户（投稿前） |
