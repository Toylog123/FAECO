...
> 施工契约：`FAECO_V2_IMPL_CONTRACT_20260923.md`（G2/G3/G4 接口冻结，2026-09-23）——**施工前必读**
...
> **✅ 已裁定（2026-09-23）：路径 B**。架构含义 = **`SearchState` 为唯一运行时状态所有者**（`refinement_weights` 只表示代价参数 $\boldsymbol\lambda$，EMA 历史另存 `FailureFeedbackState`）。精确规格见 `FAECO_V2_TECH_DESIGN_20260923.md` §2；实施步骤见其 §8 步骤 0a，前置为 G2/G3/G4（状态与归因语义钉死——**规格已写成待确认**，见 `FAECO_V2_IMPL_CONTRACT_20260923.md`）。

两条路径都必须同时修好 **新-8 的 schema 记录缺口**（`enable_feedback`/`strategies`/`init_weights`/`toolchain`），否则重做的三臂消融会重复"结论无可追溯证据"。

**建议的立即动作（按序）**：~~先钉 G2/G3/G4~~（规格已写成，待确认）→ **0a 代码基对账（路径 B，第一阶段行为不变）** → legacy regression → 0b 事实修正（OI-009/010/012）→ 1-A Mixed-Fixed 三臂 → 1-B 失败率反馈 → 2-A ΔDepth + S。

> **当前状态（2026-09-23）**：设计冻结，进入实现准备；G2/G3/G4 为最后 implementation gate，**规格已写成，待确认后即可正式编码**。
