...
**真实验证（2026-08-04 晚）——从离线预测升级为真实修复**：对每个目标电路，用其他 7 电路 trial 归纳的 leave-one-out 决策表（覆盖率 s27/s420/s641 100%、s382 90%、s713 96%、s820 85%、s832 95%、s953 67%），真实跑外环修复（beam=1 + 决策层 + early-stop，batch 并行 wall 22.4s）：**8/8 全部成功，STA 合计 27 与用本电路全数据表完全相同（逐电路 4/8/3/1/1/3/3/4），final WNS 逐电路一致**（-0.21/-0.93/-1.75/-1.85/-1.85/-1.36/-1.12/-1.38）。跨电路经验复用从“离线预测 top-2 命中 94-98%”升级为“**真实修复零损失**”：从未见过目标电路 trial 的决策表，达到与全数据表同等效率与质量。诚实边界：s953 覆盖率 67% 仍成功（关键瓶颈 cell 在表内）；表来自 8 电路内环 trial，跨工艺/跨库泛化未验证。脚本：scripts/build_leave_one_out_tables.py；实验产物 experiments/20260804_loocv_real/（A-only）。
