...
**修复前版本因 buffer 多驱 bug（同一原始网多 sink 共用 net+__buf，多 buffer 并联驱动一网）产生非法网表，OpenSTA 结果虚高，旧版数据不可信。修复（commit 5c70b30：新网名嵌入 instance+pin、输出引脚跳过、每轮重解析）后重跑：5/8 电路完成，s382 收敛到 MET（+0.02），其余接近收敛；全部 netlist_audit ok（0 多驱）。**
