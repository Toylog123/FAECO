...
| 电路 | baseline WNS | 旧版(单路径) | 新版(多路径) | 改善 | 策略 |
|---|---|---|---|---|---|
| s27 | -0.28 | -0.01 | **+0.03 (MET)** | +0.31 | 5B |
| s382 | -0.94 | -0.80 | **-0.10** | +0.84 | 47B+2G |
| s420 | -1.78 | -0.23 | **-0.08** | +1.70 | 30B |
| s641 | -1.86 | -0.36 | **+0.01 (MET)** | +1.87 | 32B |
| s713 | -1.86 | -0.57 | **+0.01 (MET)** | +1.87 | 33B+2G |
| s820 | -1.42 | -1.11 | **-0.63** | +0.79 | 67B+2G+2R |
| s832 | -1.15 | -1.11 | **-0.47** | +0.68 | 51B+4G+2R |
| s953 | -1.48 | -1.05 | **-0.19** | +1.29 | 127B+2G |

**8/8 电路全部大幅改善，3 个收敛到 MET（s27 +0.03、s641/s713 +0.01）**。关键改进：runner 显式收集全部违例路径实例（multi_path=True，baseline/round 用 `report_checks -slack_max 0`），旧版只覆盖单条关键路径（6 实例），严重低估修复潜力；tns-aware 接受逻辑记录 TNS 数据，但本次改善主要由多路径贪心产生。
