...
| X19 | 多轮 refinement 与真实 recovery 口径 | done | P0 | 每轮候选重新分类，记录 residual failures、停止原因和首次恢复轮次；batch recovery 不再按 replacement applied 代理 | **已实现（2026-08-04）**：src/rseco/refinement_loop.py simulate_refinement_loop（cut->classify->refine_weights->re-cut，支持 enable_feedback 消融开关 + WNS 驱动成功标准）+ flow.py run_multi_iteration_case；**复核修正**：F1 结构等价与 F4 reduction>=1 判据互斥致成功路径不可达、F1/F2/F4 反馈未进排序 cost（惰性）、「F3 被反馈消除」不可复现（实测 c432/c499/c880 每轮只有 F4）；已注入 functional equivalence checker 打通成功路径（合成 case 验证，3 项回归测试）；161 测试全绿；真实重综合网表 + 权重全参与排序 + WNS 接入为后续项 |
