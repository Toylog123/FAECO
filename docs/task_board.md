...
| X19 | 多轮 refinement 与真实 recovery 口径 | done | P0 | 每轮候选重新分类，记录 residual failures、停止原因和首次恢复轮次；batch recovery 不再按 replacement applied 代理 | **已实现（2026-08-04）**：src/rseco/refinement_loop.py simulate_refinement_loop（cut->classify->refine_weights->re-cut，支持 enable_feedback 消融开关 + WNS 驱动成功标准）+ flow.py run_multi_iteration_case；**两处缺陷已修复**：(1) F1 结构等价与 F4 reduction>=1 判据互斥致成功路径不可达——注入 functional equivalence checker 打通（合成 case 验证）；(2) F1/F2/F4 权重未进排序 cost 的反馈惰性——build_weighted_cut_graph 重写为 F1-F5 全权重进入节点成本，weighted_cut_candidates 真正求解加权 s-t min-cut（tests/test_weighted_cut_feedback.py 6 项）；167 测试全绿；真实重综合网表 + WNS 接入为后续项 |
...
| N31-05 | failure-aware 混合修复（G/R/B 自适应切换）实现与验证 | done | P1 | src/rseco/{gate_sizing,logic_rewrite,buffer_insertion,strategy_selector}.py + scripts/run_hybrid_repair.py + 8 电路全部改善 | **8/8 完成（2026-08-04）**：s27 -0.01、s382 +0.02(MET)、s420 -0.01、s641 -0.02、s713 -0.01、s820 -0.20、s832 -0.47、s953 -0.09，全部 netlist_audit ok；决策层（strategy_selector + 12205 trials 决策表）s832 -0.45/-15% trials；leave-one-out 跨电路 top-2 命中 94-98%；buf_8/16 负面结论（大 buffer pre-layout 有害）；探索守卫修复 s820 迁移失败；158 测试全绿 |
