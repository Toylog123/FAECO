...
| N31-05 | failure-aware 混合修复（G/R/B 自适应切换）实现与验证 | done | P1 | `src/rseco/{gate_sizing,logic_rewrite,buffer_insertion}.py` + `scripts/run_hybrid_repair.py` + 4 电路全部改善 | **G/R/B 多轮混合修复完成（2026-08-03，待提交）**：R 策略（`logic_rewrite.py`，跨 family 等价重写，如 `lpflow_inputiso1p_1`→`or2_4`）+ B 策略（`buffer_insertion.py`，高扇出节点插 buffer 分担输入电容）+ G 策略（gate sizing）三者经 OpenSTA 实测、只接受严格改善 WNS 的改动，多轮重解析关键路径持续收敛；period 0.5ns 最终 **4/4 电路大幅改善**：s27 -0.28→**-0.01**、s382 -0.94→**-0.80**（R/B 突破 G 单手段天花板）、s420 -1.78→**-0.23**、s641 -1.86→**-0.57**；B 策略在 pre-layout 下有效（高扇出输入电容分担）推翻预期无效假设，成为主力策略；20 个 R/G/B 单元测试全绿；实验产物含每候选独立记录（可审计）; audit fixed (commit a82ecf8: round/trial_id, one accepted per inst); 8c verified: s713 -1.86->-0.57, s820 -1.42->-1.11, s832 -1.15->-1.11, s953 -1.48->-1.05 (dff blackbox preprocess + buf_4) |
