...
| N31-05 | Gate sizing 修复器（方向 B 混合修复 G 策略）实现 | in_progress | P1 | `src/rseco/gate_sizing.py` + `scripts/run_gate_sizing.py` + ISCAS89 sequential 3/4 真实改善 | **审查修复后真实改善（2026-08-03，commit `ef74d1d`，108 测试全绿，已 push）**：修复 RC1（`link_design` 用 parent 目录名致 candidate link 全失败 → 改从 Verilog module 声明解析 top_module）+ RC2（逻辑深度选门≠OpenSTA 关键路径 → 改从 `report_checks` 解析关键路径实例）+ 贪心试全部更大尺寸选最优；修正后 ISCAS89 period 0.5ns **3/4 电路 WNS 改善**（s27 -0.28→-0.20、s420 -1.78→-1.41、s641 -1.86→-1.43），s382 0.0（瓶颈 `lpflow_inputiso1p_1` 无更大尺寸）→ 印证 failure-aware 混合修复（R/B）必要性；配合 `docs/engineering/n31_05_sequential_eco.md` 推进 |
