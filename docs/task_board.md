...
| N31-05 | Gate sizing 修复器（方向 B 混合修复 G 策略）首轮实现 | in_progress | P1 | `src/rseco/gate_sizing.py` + `scripts/run_gate_sizing.py` + ISCAS89 sequential 实验待改进 | **首轮实现（2026-08-03）**：SKY130 cell 网表解析 + 逻辑深度 critical gate 选择 + 更大尺寸候选 + 贪心放大；s27/s382/s420 实测 WNS 无改善、`sized_gates` 全空，原因审查中（逻辑深度≠OpenSTA 关键路径 / 尺寸 delay 差异<WNS 精度 / 电路太小）；配合 `docs/engineering/n31_05_sequential_eco.md` 推进；结论暂不入论文主表 |
