...
### 10.4 ?? WNS ???OpenSTA ???2026-08-04 ??

10.2 ?"?????? patch ???? OpenSTA"???????

- **`src/rseco/opensta.py` ?? `run_opensta_sequential`**?? SKY130 Liberty + mapped.v + `create_clock -name clk -period <p> [get_ports CK]`???? WSL2?`wsl.exe -d Ubuntu -- /usr/local/bin/sta`???? slack/wns/tns
- **`src/rseco/real_wns.py` ?? `RealWnsEvaluator`**?`(patch, weights) -> {wns, improved}`???? cut ??????????????? SKY130 ???????????report_checks ????? R??????/G??????/?? B?buffer????????? OpenSTA ???`--workers` ????????? WNS ??
- **`flow.py` evaluator ?????????? 8 ??? cut**???? `candidates[0]`?????? WNS ????????????? F4 ? refine_weights ??
- **???????3 ????**?(1) ???? DFF ?????reg-to-reg ????DFF ?????? `cut._logic_depths` ????(2) `flow.run_multi_iteration_case` ?????????? fallback ??? 0???????? WNS??(3) ?????? checker??????????????????? WNS ???????

**4/4 ISCAS89 ???????period 0.5ns???????????**?
...
??"cut???????????"?????s382/s27/s420 ???????? min-cut/?? cut???????? 5 ????s382 fixed ?????? `_071_` R ?? -0.92?s641 ???????? min-cut?? -1.85?**?????? + ???2026-08-04 ??**?beam=1 ??????????ON/OFF ? s382 ?????? min-cut ???? root?critical_coverage_reward=9 ???? `_071_`?beam=8 ??????????????F4 ????? `critical_path_cover` cut???????????F4 ??????"?????"???????**4 ?? beam=1 ?????? OpenSTA?**?

| ?? | ?? ON | ?? OFF | ?? patch |
|---|---|---|---|
| s27 | ? 2 ? -0.21 | 8 ??? | critical_path_cover?G nor3b_2?|
| s382 | ? 2 ? -0.92 | 8 ??? | critical_path_cover?R or2_4?|
| s420 | ? 2 ? -1.51 | 8 ??? | critical_path_cover?G nor4_4?|
| s641 | ? 1 ? -1.85 | ? 1 ? -1.85 | weighted_st_min_cut?????????????|

**??????????? F4 ???? recovery?3/4 ?? ON/OFF ????**??s27/s382/s420 ?? OFF ????????? ON ? 2 ?? critical_path_cover ??????????beam>1 ?????????????????s382 ? 5 ?????????? beam=1 ??????????s641 ???????????????????

**beam??????s382?2026-08-04 ????????"????????"**?

| beam | ?? ON | ?? OFF |
|---|---|---|
| 1 | ?? iter2?12 ? STA?| 8 ????16 ? STA?|
| 2 | ?? iter2?14 ? STA?| 8 ????32 ? STA?|
| 4 | ?? iter2?20 ? STA?| 8 ????80 ? STA?|
| 8 | ?? iter1?25 ? STA?| ?? iter1?25 ? STA?|

beam?{1,2,4} ?? OFF ????STA ????ON ????beam=8 ?????????????**beam=2+?? ON?14 ? STA??? beam=8?25 ???? -0.92?STA ?? -44%**??????? beam ????????????"????????"??????

**??? 4 ???2026-08-04 ??**??????

| ?? | beam=1/2 OFF | beam=1/2 ON | ? beam OFF | ?? |
|---|---|---|---|---|
| s27 | ???24/48 STA?| ???8/11 STA?| beam=8 ???46?| ???? beam?2?STA -76% |
| s382 | ???16/32 STA?| ???12/14 STA?| beam=8 ???25?| ???? beam?2?STA -44% |
| s420 | ???8/16 STA?| ???12/13 STA?| beam?4 ???122?| ???? beam?2?STA -89% |
| s641 | ???3 STA?| ???3 STA?| ???3?| ?????????? |

???? 3/4 ???????????? beam ? ?4-8 ?? ?2?STA ?? -44%~-89%?s641 ????????????????? min-cut ???????????????s420 ? beam ? 122 ? STA ? random_cut ??? max_instances=8 ????????????????????beam ???????????????"? beam ???????"????"??????????"?
