...
| R31-01 | Stage B CEC limitation 在 SKY130 Liberty 中不可绕过 | 中 | 中 | P1 | **已解决（2026-08-03）**：ABC `cec` 无法对 Liberty subcircuit 建 model（连极简 Liberty 也失败），但改用 **Yosys miter + SAT** 等价证明路径（`scripts/make_liberty_cells_v.py` 从 Liberty function 生成 assign-style cells.v + `scripts/verify_epfl_mapping_sat.py`），**8/8 EPFL case 等价证明 SUCCESS**（`sat -prove-asserts` 不可满足）。CEC limitation 经此路径关闭；`verify_epfl_mapping_sat.py` 产物 `tmp/sat_verify/sat_equivalence_summary.json` | mitigated |
