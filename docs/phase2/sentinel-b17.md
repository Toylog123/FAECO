...
The D-drive attempt is the most complete but was stopped mid-run with no `sentinel_manifest.json`, so it cannot be used to claim a b17 result. The required action remains a fresh, uninterrupted rerun on D (do not run on C; it fills the system drive), with `git_head_sha`, tool versions, WNS history, and SEC populated in the manifest.
